...
class NotionClient:
    ...
    def request(self, method: str, path: str, *, params: Optional[Dict[str, Any]] = None,
                json_body: Optional[Dict[str, Any]] = None, stream: bool = False) -> requests.Response:
...
def block_children_to_md(client: NotionClient, blocks: List[Dict[str, Any]], page_ctx: Dict[str, Any],
                         indent: int = 0) -> Tuple[str, List[str]]:
    md_lines: List[str] = []
    ...
    while i < len(blocks):
        ...
        if block.get("has_children") and block_type not in ("toggle", "table", "bulleted_list_item", "numbered_list_item", "to_do"):
            children = get_blocks(client, block["id"])
            child_md, child_pages = block_children_to_md(client, children, page_ctx, indent + 1)
            if child_md:
                md_lines.append(child_md.rstrip())
...
def compute_page_path(page_id: str, pages: Dict[str, Dict[str, Any]],
                      databases: Dict[str, Dict[str, Any]], data_sources: Dict[str, Dict[str, Any]],
                      page_paths: Dict[str, str], db_paths: Dict[str, str],
                      slug_registry: Dict[str, Dict[str, int]], root: str) -> str:
    ...
    if parent_type == "workspace" or not parent_type:
        ...
        parent_path = compute_page_path(parent_id, pages, databases, data_sources, page_paths, db_paths, slug_registry, root)
        ...
        if db_id and db_id in databases:
            parent_dir = compute_db_path(db_id, pages, databases, data_sources, page_paths, db_paths, slug_registry, root)
        else:
            parent_dir = root
    else:
        parent_dir = root
...
def compute_db_path(db_id: str, pages: Dict[str, Dict[str, Any]],
                    databases: Dict[str, Dict[str, Any]], data_sources: Dict[str, Dict[str, Any]],
                    page_paths: Dict[str, str], db_paths: Dict[str, str],
                    slug_registry: Dict[str, Dict[str, int]], root: str) -> str:
    if not db_id:
...
def main() -> None:
    ...
    pages_list = list_all(client, "/search", {"page_size": 100, "filter": {"property": "object", "value": "page"}})
    for page in pages_list:
        pages[page["id"]] = page

    data_source_list = list_all(client, "/search", {"page_size": 100, "filter": {"property": "object", "value": "data_source"}})
    ...
    while export_queue:
        ...
        page = pages[page_id]
        processed += 1
        if args.log_every > 0 and (processed == 1 or processed == total_pages or processed % args.log_every == 0):
            title = get_page_title_plain(page)
            print(f"[{processed}/{total_pages}] Exporting: {title}")
        page_path = compute_page_path(page_id, pages, databases, data_sources, page_paths, db_paths, slug_registry, output_root)
        page_dir = os.path.dirname(page_path)
        assets_dir = os.path.join(page_dir, "assets")
        ensure_dir(page_dir)

        # Prepare links for child pages if known
        ...
        for child_id in pages.keys():
            ...
            if parent.get("type") == "page_id" and parent.get("page_id") == page_id:
                child_path = compute_page_path(child_id, pages, databases, data_sources, page_paths, db_paths, slug_registry, output_root)
    ...
    # Export database index pages
    for db_id, db in databases.items():
        db_dir = compute_db_path(db_id, pages, databases, data_sources, page_paths, db_paths, slug_registry, output_root)
        ensure_dir(db_dir)
